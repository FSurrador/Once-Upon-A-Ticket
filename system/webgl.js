let print = new Boolean(true);
let fail = new Boolean(false);
...
function getMeta(url){
    ...
    img.addEventListener("load", function() {
        ...
        dados[12] = this.naturalHeight*800/this.naturalWidth;
        console.log("DADOS12: "+dados[12]);
        //console.log("ALTURAAA:"+altura);
...
function render_progress_bar (gl, current_phase, face_predictions, win_w, win_h)
{
    ...
    if(fail==false) {
        let fail2 = document.getElementById("fail");
        fail2.append("I'm sorry, I failed to generate a good ticket for you. Click here to try again!");
...
function startWebGL()
{
    ...
    const camtex = GLUtil.create_image_texture (gl, dados[3]);

    //const camtex = GLUtil.create_video_texture (gl, "./assets/just_do_it.mp4");
    const imgtex = GLUtil.create_image_texture2 (gl, dados[7]);



    let masktex = GLUtil.create_image_texture2 (gl, dados[3]);
    let masktex_next;
    ...
    async function render (now)
    {
        ...
        stats.end();
        if (print) {
            console.log("------- printing image start -------");
            printImage();
            console.log("------- printing image end -------");

            localStorage.setItem("arraydata", JSON.stringify(dados));


            //console.log(getMeta(dados[2]));
...
function printImage() {
    ...
    let canvasfinal = document.getElementById("glcanvas");
    let image_data_url_final = canvasfinal.toDataURL('image/jpeg');
    dados[8] = image_data_url_final;
    console.log("FINAL",image_data_url_final);
    console.log(dados);
    const imagefinal = document.createElement("img");
    imagefinal.src = image_data_url_final;
...
function initiatePrinting() {
    console.log("------- initiatePrinting begin -------");
    print = true;
}
